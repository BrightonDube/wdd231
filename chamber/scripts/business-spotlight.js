  ...
  fetch('./data/members.json')
    ...
    .then(data => {
      const eligibleMembers = data.filter(
        member => member['membership level'] >= 2
...
function displayRandomBusinesses(members) {
  ...
  const selectedBusinesses = shuffleArray(members).slice(
    0,
  ...
  businessCards.forEach((card, index) => {
    ...
    if (business) {
      ...
      card.querySelector(
        '.address'
      ).textContent = `Address: ${business.addresses}`;
      card.querySelector(
        '.phone'
      ).textContent = `Phone: ${business['phone numbers']}`;
      ...
      card.querySelector(
        '.membership'
      ).textContent = `Membership Level: ${membershipLevelText}`;
